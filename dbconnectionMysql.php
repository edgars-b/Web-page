<?php

class PdoDB
{

    private $dbhost = 'mysql:host=localhost;dbname=';
    private $dbname = 'Riga_Coding';
    private $dbuser = 'edroot';
    private $dbpass = 'allstarpass';

    private $writeLoginSQL = "INSERT INTO users (`login`, `password`) VALUES (?, ?)";
    private $getUserLoginId = "select login, creation_date from Riga_Coding.users where id = ?";

    public function __construct()
    {
        try {
            $this->db = new PDO($this->dbhost . $this->dbname, $this->dbuser, $this->dbpass);
        } catch (PDOException $e) {
            $this->db = null;
        }
    }

    public function getUserData($userID)
    {
        $response = [];
        if ($this->db) {
            $statement = $this->db->prepare($this->getUserLoginId);
            if ($statement->execute([$userID])) {
                while($row = $statement->fetch(PDO::FETCH_ASSOC)){
                    $response[] = $row;
                }
            }
            return $response;
        }
        return null;
    }

    public function getData($query, $fields, $fetch_type = 2)
    {
        $res = [];
        if ($this->db) {
            $stmt = $this->db->prepare($query);
            if ($stmt->execute($fields)) {
                while ($row = $stmt->fetch($fetch_type)) {
                    $res[] = $row;
                }
            }
            return $res;
        }
        return null;
    }

    public function writeLogin($login, $password)
    {
        $password = password_hash($password, PASSWORD_DEFAULT);
        $stmt = $this->db->prepare($this->writeLoginSQL);
        if ($stmt->execute([$login, $password])) {
            return true;
        }
        return false;
    }

}
