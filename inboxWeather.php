...
if (file_exists('inboxWeather.json') and filemtime('inboxWeather.json') > time() - 300) {
    ...
    file_put_contents('updateTime.txt', 'File update time: ' . date('Y-m-d H-i-s', filemtime("inboxWeather.json")) . "\n");
    file_put_contents("updateTime.txt", "Update call time: " . date("Y-m-d H-i-s", time()), FILE_APPEND);
    echo $info;
}
